...
import { Shield, KeyRound, Mail, AlertCircle, RefreshCw } from 'lucide-react'
...
const LoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || 'Failed to authenticate user credentials.')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="w-full min-h-[calc(100vh-73px)] flex items-center justify-center p-6 bg-mesh">
      <div className="w-full max-w-md glass-card p-8 flex flex-col relative overflow-hidden group">
        ...
        <div className="flex flex-col items-center text-center mb-8">
          ...
          <p className="text-slate-400 text-xs mt-1.5">Sign in to access ScreenShield forensic hub</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          ...
          <button type="submit" disabled={loading} className="btn-primary w-full mt-2 flex items-center justify-center gap-2">
            {loading ? <RefreshCw className="w-4.5 h-4.5 animate-spin" /> : 'Authenticate Credentials'}
          </button>
        </form>
